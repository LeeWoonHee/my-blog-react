...
const Carousel = () => {
    ...
    if(next){
      if(prevIndex === false){
        setSlideData(() => {
          ...
          postData.push(temp[currentIndex-1])
  ...
  useEffect(() => {
   ...
   openAnimation.to('.carousel-slide-active', {
      ...
      .fromTo('.background-image-active img',{
        borderRadius: '25px',
      }, {
        borderRadius: 0,
        ease: 'Power1.easeOut',
  ...
  useEffect( () => {
    if(next && timeline.isActive() === false){
      ...
      timeline
        ...
        .fromTo('.background-image-active+.background-image img',{
          borderRadius: 0,
        }, {
          borderRadius: '25px',
          duration : 1
  ...
  return (
    <div className="carousel-container">
      ...
      {postData.map((banner, index) => (
        ...
        index === currentIndex ? 'background-image-active' : ''
        }`}
        >
        <div className={`virtual-bg ${
        index === currentIndex ? 'virtual-bg-active' : ''
        }`}></div>
